...
const ListIncident = ({ staffList, ...props }) => {
    let insidentList = staffList.map((item) => {
    ...
    function showConfirm(id) {
        confirm({
          title: 'Do you Want to delete these incident?',
    ...
    const deleteIncident = async (id) => {
        await axios.delete('/api/incident/'+id ,props.header)
        .then((response) => {
    ...
    return (<Table
        rowKey='id'
        dataSource={insidentList}
        loading={props.loading}
        columns={columns}
        expandable={{
          expandedRowRender: record => (
            <List
              header={<Title level={4}>Incidents</Title>}
              itemLayout="horizontal"
              bordered
              dataSource={record.incidents}
              renderItem={item => {
                return <List.Item
                  key={item._id}
                  actions={[
